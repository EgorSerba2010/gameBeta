...
const WHEEL_PRIZES_CONFIG = [
    { id: 1, name: "💰 +15 мин Капитал",  degMin: 0,   degMax: 60,  icon: "💰" },       // 1 сектор: мешок с деньгами
    { id: 6, name: "🔥 Крутая Карта", degMin: 60,  degMax: 120, icon: "⬛❓" },     // 2 сектор: секретная чёрная карта
    { id: 5, name: "🧧 +3 Алмаза!",    degMin: 120, degMax: 180, icon: "💎💎💎" },   // 3 сектор: 3 алмаза друг с другом
    { id: 4, name: "🖱️ Клики Х3 на 5 минут!",     degMin: 180, degMax: 240, icon: "🖱️ x3" },    // 4 сектор: курсор мыши и надпись х3
    { id: 3, name: "🔄 Ещё Прокрут!",   degMin: 240, degMax: 300, icon: "🔄" },       // 5 сектор: то же эмодзи респина, что и в массиве
    { id: 2, name: "⚡ Золотой Век +1 час",   degMin: 300, degMax: 360, icon: "🪙 x2" }     // 6 сектор: монетка и надпись х2
];


// Переменная для фиксации текущего угла поворота колеса, чтобы анимация не сбрасывалась
let currentWheelRotationDegrees = 0;
let isWheelSpinningNow = false; // Блокировка повторного клика во время кручения
...
let clickBoostUntil = 0; // Время в мс, до которого активен буст х3 на клики

let wheelCharges = 3;       // Текущее количество зарядов (от 0 до 3)
let lastWheelChargeTime = 0; // Время в мс, когда начал копиться следующий заряд
const WHEEL_CHARGE_COOLDOWN = 5 * 60 * 1000; // Кулдаун восстановления 1 заряда: 5 минут в мс
...
function saveGame() {
    ...
    const gameState = { 
        ...
        wheelCharges: wheelCharges,
        lastWheelChargeTime: lastWheelChargeTime,
        clickBoostUntil: clickBoostUntil
    };
...
function loadGame() {
    ...
    if (savedData) {
        try {
            ...
            clickBoostUntil = gameState.clickBoostUntil || 0;

            // ИСПРАВЛЕННОЕ ВОССТАНОВЛЕНИЕ ЗАРЯДОВ ИЗ ОФФЛАЙНА
            wheelCharges = (gameState.wheelCharges !== undefined) ? gameState.wheelCharges : 3;
            lastWheelChargeTime = gameState.lastWheelChargeTime || 0;

            // Если бак не полный, считаем сколько минут прошло с последнего сохранения
            if (wheelCharges < 3 && lastWheelChargeTime > 0) {
                const timePassedMs = Date.now() - lastWheelChargeTime;
                const earnedCharges = Math.floor(timePassedMs / WHEEL_CHARGE_COOLDOWN);
                
                if (earnedCharges > 0) {
                    wheelCharges = Math.min(wheelCharges + earnedCharges, 3);
                    // Если бак заполнился до максимума, сбрасываем таймер
                    if (wheelCharges === 3) {
                        lastWheelChargeTime = 0;
                    } else {
                        // Если не заполнился, сдвигаем таймер на остаток времени
                        lastWheelChargeTime += earnedCharges * WHEEL_CHARGE_COOLDOWN;
                    }
                }
            }
...
function runDynamicPassiveIncomeLoop() {
    ...
    const speedBonus = (upgrades.autoIncomeSpeed ? upgrades.autoIncomeSpeed.level : 0) * 500;
    let currentInterval = Math.max(PASSIVE_INCOME_INTERVAL - speedBonus, 500);

    // Сохраняем идентификатор таймаута перед запуском
    passiveIncomeTimeoutId = setTimeout(runDynamicPassiveIncomeLoop, currentInterval);
}
...
// Автоматически создаем графические иконки вместо старых скучных цифр 1-6
function initFortuneWheelSectors() {
    const wheelCircle = document.getElementById('fortune-wheel-canvas');
    if (!wheelCircle) return;

    wheelCircle.innerHTML = ''; // Очищаем колесо

    // Создаем массив иконок строго по порядку секторов (от 1 до 6)
    const sectorIcons = [
        WHEEL_PRIZES_CONFIG.find(p => p.id === 1).icon,
        WHEEL_PRIZES_CONFIG.find(p => p.id === 6).icon,
        WHEEL_PRIZES_CONFIG.find(p => p.id === 5).icon,
        WHEEL_PRIZES_CONFIG.find(p => p.id === 4).icon,
        WHEEL_PRIZES_CONFIG.find(p => p.id === 3).icon,
        WHEEL_PRIZES_CONFIG.find(p => p.id === 2).icon
    ];
    
    sectorIcons.forEach((iconText, index) => {
        const textNode = document.createElement('div');
        textNode.className = 'wheel-sector-text';
        textNode.textContent = iconText;
        
        // Корректируем размер шрифта для длинных иконок, чтобы они красиво влезали в треугольники секторов
        if (iconText.length > 2) {
            textNode.style.fontSize = '15px'; 
            textNode.style.paddingTop = '10px'; // Чуть ниже опускаем, чтобы не прижималось к краю
        } else {
            textNode.style.fontSize = '24px'; // Одиночные эмодзи типа 💰 или 🔄 делаем крупными и сочными
            textNode.style.paddingTop = '10px';
        }
        
        // Каждая плашка занимает ровно 60 градусов, иконка встаёт ровно по центру (+30 градусов)
        const rotateAngle = (index * 60) + 30; 
        textNode.style.transform = `rotate(${rotateAngle}deg)`;
        
        wheelCircle.appendChild(textNode);
    });
}

// Тестовая функция кручения колеса (Пока БЕЗ рекламы и БЕЗ списания зарядов)
function claimWheelSpin(isFreeBonusSpin = false) {
    const wheelCircle = document.getElementById('fortune-wheel-canvas');
    const screenDisplay = document.getElementById('wheel-screen-display');
    const spinBtn = document.getElementById('spin-wheel-btn');

    if (!wheelCircle || !screenDisplay) return;

    isWheelSpinningNow = true;
    if (spinBtn) spinBtn.disabled = true;
    screenDisplay.textContent = "КРУТИМ БАРАБАН...";

    // 1. Честно выбираем случайный сектор от 1 до 6
    const luckyId = Math.floor(Math.random() * 6) + 1;

    // 2. Рассчитываем угол упругой остановки с живым люфтом
    const sectorCenterAngle = ((luckyId - 1) * 60) + 30;
    const randomShift = (Math.random() * 36) - 18;
    const finalSectorAngle = sectorCenterAngle + randomShift;
    const targetAngle = 360 - finalSectorAngle;

    // 3. Абсолютный поворот барабана только вперед
    const baseAngle = currentWheelRotationDegrees % 360;
    const degreesToTarget = (targetAngle >= baseAngle) ? (targetAngle - baseAngle) : (360 - baseAngle + targetAngle);
    const extraTurns = 360 * 5;
    currentWheelRotationDegrees += extraTurns + degreesToTarget;

    wheelCircle.style.transform = `rotate(${currentWheelRotationDegrees}deg)`;

    // 4. Ждем 4 секунды до мягкого торможения
    setTimeout(() => {
        // Находим конфигурацию выигранного сектора
        const prize = WHEEL_PRIZES_CONFIG.find(p => p.id === luckyId);
        screenDisplay.textContent = prize.name.toUpperCase();
        
        if (typeof buySound !== 'undefined') {
            buySound.currentTime = 0;
            buySound.play().catch(err => console.log(err));
        }

        // === НАСТОЯЩЕЕ НАЧИСЛЕНИЕ НАГРАД КОЛЕСА ===
        switch(luckyId) {
            case 1: // 💰 Сектор 1: Быстрый капитал (х15 минут денег)
                const activeCards = document.querySelectorAll('.drag-item:not(.absorb-anim)');
                let currentPPSValue = 0;
                activeCards.forEach(card => { currentPPSValue += Number(card.getAttribute('data-value') || 0); });
                const currentPPS = currentPPSValue / (PASSIVE_INCOME_INTERVAL / 1000);
                const moneyEarned = Math.max(Math.round(currentPPS * 900), 500); // Минимум 500$
                
                updateBalance(moneyEarned);
                spawnFloatingText(`+${formatNumber(moneyEarned)} $`, "50%", "50%");
                break;

            case 2: // ⚡ Сектор 2: Золотой Век (+30 минут х2 буста)
                const baseGoldTime = Math.max(doubleIncomeUntil, Date.now());
                doubleIncomeUntil = baseGoldTime + (60 * 60 * 1000); // +30 минут в мс
                calculatePPS();
                spawnFloatingText("Х2 ДОХОД +1 ЧАС!", "50%", "50%");
                break;

            case 3: // 🔄 Сектор 3: Ещё один прокрут! (Респин)
                spawnFloatingText("БЕСПЛАТНЫЙ ХОД!", "50%", "50%");
                // ИСПРАВЛЕНИЕ: Даем игроку возможность крутануть еще раз бесплатно прямо сейчас
                setTimeout(() => {
                    claimWheelSpin(true); 
                }, 7050); // Запуск строго после полной остановки колеса
                break;
            ...
            case 4: // 🖱️ Сектор 4: Клики Х3 на 5 минут
                ...
                if (typeof spawnFloatingText === 'function') {
                    spawnFloatingText("ЯРОСТЬ ТАПОВ: КЛИКИ Х3!", "50%", "50%");
                }
                break;

            case 5: // 🧧 Сектор 5: Алмазный Джекпот (+3 Алмаза)
                diamonds += 3;
                if (diamondsValueEl) diamondsValueEl.textContent = formatNumber(diamonds);
                spawnFloatingText("+3 💎 ДЖЕКПОТ!", "50%", "50%");
                break;

            case 6: // 🔥 Сектор 6: Теневая Карта (Случайный сильный персонаж)
                // 1. Берем упорядоченный список всех номиналов в игре
                const globalNominals = Object.keys(CARDS_DATABASE).map(Number).sort((a, b) => a - b);
                // 2. Находим индекс максимальной открытой карты за всё время
                const maxDiscovered = Math.max(...discoveredCards, 1);
                const maxIndex = globalNominals.indexOf(maxDiscovered);

                let jackpotCardValue = 1;
                // 3. Если открыто хотя бы 3 карты (например, до тира "4" или "8")
                if (maxIndex >= 2) {
                    // Создаем пул строго из двух карт: предпоследней [maxIndex - 1] и предпредпоследней [maxIndex - 2]
                    const elitePool = [globalNominals[maxIndex - 1], globalNominals[maxIndex - 2]];
                    jackpotCardValue = elitePool[Math.floor(Math.random() * elitePool.length)];
                } else {
                    jackpotCardValue = globalNominals[0];
                }
                
                // ИСПРАВЛЕНИЕ КАРТЫ: Проверяем, какому полю принадлежит выигрыш
                const rewardTargetField = getFieldForValue(jackpotCardValue);

                if (rewardTargetField === currentField) {
                    // Ситуация А: Карта подходит для текущего экрана. Спавним вживую!
                    if (typeof createCard === 'function') createCard(jackpotCardValue);
                    if (typeof checkCardUnlocks === 'function') checkCardUnlocks(jackpotCardValue); 
                    if (typeof spawnFloatingText === 'function') {
                        spawnFloatingText(`🎁 ПЕРСОНАЖ ${jackpotCardValue}!`, "50%", "50%");
                    }
                } else {
                    // Ситуация Б: Карта для скрытого поля. Записываем напрямую в localStorage
                    let allFieldsCards = { field_1: [], field_2: [], field_3: [] };
                    const savedData = localStorage.getItem('clicker_game_save');
                    if (savedData) {
                        try {
                    ...
                    const targetFieldKey = `field_${rewardTargetField}`;
                    if (allFieldsCards[targetFieldKey].length < MAX_CARDS) {
                        allFieldsCards[targetFieldKey].push({
                            value: jackpotCardValue,
                            left: `${getRandomPercent(5, 75)}%`,
                            top: `${getRandomPercent(5, 80)}%`
                        });
                    }
                    ...
                    // Сообщаем игроку, на каком поле искать сильного персонажа
                    alert(`🎁 Персонаж ${formatNumber(jackpotCardValue)} успешно отправлен на Поле ${rewardTargetField}!`);
                }
                break;
        }
        // =========================================
...
function updateWheelEnergyTimerUI() {
    const energySpan = document.getElementById('wheel-energy-spans');
    const timerText = document.getElementById('wheel-timer-text');
    const spinBtn = document.getElementById('spin-wheel-btn');

    if (!energySpan || !timerText) return;

    // Если зарядов меньше максимума, и таймер еще не был запущен
    if (wheelCharges < 3 && lastWheelChargeTime === 0) {
        lastWheelChargeTime = Date.now();
    }

    // Отрисовываем текущие заряды
    energySpan.textContent = `🔋 ${wheelCharges} / 3`;

    // Логика тиканья таймера
    if (wheelCharges < 3) {
        timerText.style.display = 'inline-block'; // Показываем таймер
        
        const elapsedMs = Date.now() - lastWheelChargeTime;
        const timeRemainingMs = WHEEL_CHARGE_COOLDOWN - elapsedMs;

        if (timeRemainingMs <= 0) {
            // Время вышло! Начисляем +1 заряд
            wheelCharges++;
            if (wheelCharges === 3) {
                lastWheelChargeTime = 0;
                timerText.style.display = 'none'; // Прячем таймер, бак полный
            } else {
                // Если еще не полный, перезапускаем таймер на следующий круг
                lastWheelChargeTime = Date.now();
            }
            saveGame();
        } else {
            // Переводим остаток времени в формат ММ:СС
            const totalSeconds = Math.floor(timeRemainingMs / 1000);
            const mins = Math.floor(totalSeconds / 60);
            const secs = totalSeconds % 60;
            const pad = (num) => num < 10 ? '0' + num : num;
            timerText.textContent = `Следующий через: ${pad(mins)}:${pad(secs)}`;
        }
    } else {
        timerText.style.display = 'none'; // Если 3/3, прячем таймер
    }

    // Блокируем или активируем кнопку в зависимости от зарядов и процесса вращения
    if (spinBtn && !isWheelSpinningNow) {
        spinBtn.disabled = (wheelCharges <= 0);
        spinBtn.textContent = (wheelCharges <= 0) ? "НЕТ ЗАРЯДОВ" : "📺 КРУТИТЬ!";
    }
}
// Главная функция запуска рулетки за просмотр ТВ рекламы
function claimWheelSpinWithAds() {
    if (isWheelSpinningNow) return;
    if (wheelCharges <= 0) {
        alert("Заряды энергии закончились! Подождите восстановления.");
        return;
    }
    if (!AdController) {
    ...
    // Блокируем кнопку на время открытия плеера
    const spinBtn = document.getElementById('spin-wheel-btn');
    if (spinBtn) spinBtn.disabled = true;

    AdController.show().then((result) => {
        lastAdWatchTime = Date.now(); // Кулдаун обычной кнопки ТВ
        // Списываем 1 заряд из бака энергии
        wheelCharges--;
        if (wheelCharges < 0) wheelCharges = 0;
        // Если это был первый потраченный заряд, фиксируем время старта регенерации
        if (wheelCharges === 2) lastWheelChargeTime = Date.now();
        saveGame();
        
        claimWheelSpin(false);

    }).catch((result) => {
        console.log("Прокрут отменен: реклама не досмотрена.", result);
        if (spinBtn) spinBtn.disabled = false;
    });
}
...
setTimeout(runDynamicPassiveIncomeLoop, PASSIVE_INCOME_INTERVAL);
initFortuneWheelSectors();
setInterval(updateWheelEnergyTimerUI, 1000);