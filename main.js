...
const WHEEL_PRIZES_CONFIG = [
    { id: 1, name: "💰 +15м Капитал",  degMin: 0,   degMax: 60 },
    { id: 6, name: "🔥 Теневая Карта", degMin: 60,  degMax: 120 },
    { id: 5, name: "🧧 +3 Алмаза!",    degMin: 120, degMax: 180 },
    { id: 4, name: "🖱️ Клики Х3!",     degMin: 180, degMax: 240 },
    { id: 3, name: "🔄 Ещё Прокрут!",   degMin: 240, degMax: 300 },
    { id: 2, name: "⚡ Золотой Век",   degMin: 300, degMax: 360 }
];
...
// Автоматически создаем текстовые плашки с номерами секторов внутри колеса
function initFortuneWheelSectors() {
    ...
    const sectorNumbers = [1, 2, 3, 4, 5, 6];
    
    sectorNumbers.forEach((num, index) => {
        const textNode = document.createElement('div');
        textNode.className = 'wheel-sector-text';
        textNode.textContent = num;
        
        // Считаем угол: каждый сектор 60°, цифра строго посередине (+30°)
        const rotateAngle = (index * 60) + 30; 
        
        // УБРАЛИ translateX(-50%), оставляем только чистое вращение вокруг центра!
    ...
    setTimeout(() => {
        ...
        switch(luckyId) {
            ...
            case 3: // 🔄 Сектор 3: Ещё один прокрут! (Респин)
                // Пока у нас нет энергии, просто информируем текстом, в будущем заряд не спишется
                spawnFloatingText("КРУТИ ЕЩЁ РАЗ!", "50%", "50%");
                break;
